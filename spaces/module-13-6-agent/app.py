...
def build_instructions(skills):
    ...
    return (
        ...
        "character_on_cooldown значит, что житель ещё занят предыдущим делом "
        "(шаг ~1 с, добыча ~10 с; остаток — в message). Инструмент досиживает "
        "такт сам, так что этот код ты увидишь редко; увидел — повтори тот же вызов."
    )
