"""Инструменты лесоруба на РЕАЛЬНОМ API игры Cognopolis — как FunctionTool LlamaIndex.

Контракт — дословно тот, что вы собрали в 11.5 и гоняли через MCP в 14.5:
узкие инструменты move / gather / get_character / get_map, конверт ответа
{result, cooldown, character} целиком, обучающие ошибки {code, message}. Новое
здесь — ЧЕМ контракт оформлен: лекция 14 называет FunctionTool «функцией с
паспортом», и паспорт собирается из самой функции — имя из имени, description
из docstring, схема аргументов из type hints (Literal превращается в enum).

Чек-лист A→H из лекции 11.5 никуда не делся, просто буквы A (имя), B
(description) и C (аргументы-enum) теперь закрывает аккуратно написанная
функция, а не отдельный класс: docstring — это и есть описание, которое увидит
модель, поэтому он написан ей, а не человеку. Буквы D (конверт целиком) и E
(ошибки как {code, message}) остаются нашей заботой — смотрите _envelope и
...
def build_world_tools(client: CognopolisClient, on_state) -> list[FunctionTool]:
    """Четыре инструмента-действия над живым миром.

    on_state — колбэк app.py: каждый конверт с `character` обновляет боковую
    панель. Инструменты возвращают строку-JSON: FunctionTool всё равно приводит
    результат к тексту для модели, а json.dumps делает это честно — конверт
    доезжает до агента ЦЕЛИКОМ и без питоньих кавычек (буква D чек-листа)."""
    ...
    # «Паспорт» каждой функции LlamaIndex собирает сам: имя, docstring, схему
    # аргументов (Literal -> enum, буква C). Ничего не дописываем руками —
    # в этом и пуант лекции про FunctionTool.
    return [
        FunctionTool.from_defaults(move),
        FunctionTool.from_defaults(gather),
        FunctionTool.from_defaults(get_character),
        FunctionTool.from_defaults(get_map),
    ]
