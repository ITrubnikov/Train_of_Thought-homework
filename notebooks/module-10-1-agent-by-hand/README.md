...
- **Модель дала ответ без тега `Final Answer:` -> `AttributeError: 'NoneType' ... 'group'`.** Получив Observation, модель часто отвечает прямым текстом, не повторяя `Final Answer:`. Тогда `re.search` не находит JSON и `.group()` падает. Обрабатываем: если в тексте нет ни действия (JSON), ни `Final Answer:` — это и есть финальный ответ, возвращаем его (`if not match: return text.strip()`).
