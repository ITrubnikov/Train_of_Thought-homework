...
- **Токен — только `.env`, env или getpass.** Флаг `ASK_TOKEN` в Блоке 5 по умолчанию `False`, чтобы `Run all` не замирал на вводе; впишите токен в `mcp-server/.env` (сервер подхватит его сам), поставьте `True` или задайте `COGNOPOLIS_TOKEN` в окружении. Токен не печатается и не сохраняется в ноутбуке — проследите, чтобы так и осталось в сдаваемой версии. Он нужен и ячейкам ноутбука (запустить сервер-подпроцесс с окружением, сделать факт-чек мимо агента), и серверу (подписывать вызовы API) — но это один токен из одного источника, и модель его не видит нигде: ни в промпте, ни в аргументах инструментов, ни в протоколе (разбор — в README папки `mcp-server/`).
- **Живой мир держит ритм, и ритм этот разный.** Шаг ~1 с, добыча ~10 с, крафт — десятки секунд (такты мира: `GET /stats` → `tact.seconds`; длительность рецепта — `GET /recipes` → `seconds`). Ранний вызов отвечает `character_on_cooldown` с точным временем ожидания. Ждать за агента умеет обёртка: действия в `cognopolis_mcp_server.py` идут через `_act`, который досиживает такт по полю `cooldown` ответа — без этого агент на длинном такте сжигает шаги на повторах. Возврат на дом `(0, 0)` сам сдаёт рюкзак (поле `banked` в ответе `move`) — отдельного `deposit` в живом мире нет.
...
Код — Apache-2.0 (см. [LICENSE](../../LICENSE) в корне репо). Мир, tools, коды ошибок и форма конверта совпадают с модулями 11.5/13.6 и учебной игрой Cognopolis; термины (три примитива, транспорт, discovery, «контракт, вынесенный в сервис») — с лекцией модуля 14.5 (часть курса «От нуля до своих агентов»).
