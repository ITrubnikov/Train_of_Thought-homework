...
### Кто и зачем держит токен

Использует токен только сам сервер: он подписывает им каждый вызов игрового API (`Authorization: Bearer`); один запущенный сервер = один житель. Если сервер запускает клиент, которому токен знать незачем (Inspector, Claude Code, Claude Desktop с заполненным `.env`), — токен живёт только у сервера. Если же сервер поднимает ваше собственное приложение (ноутбук модуля, space-агент), токен появляется и у него — но не «для авторизации агента», а по двум служебным причинам: при stdio именно запускающий передаёт окружение подпроцессу, и именно приложение делает прямые проверки мира (проба токена, факт-чек `stored` до/после) мимо агента. Модель (LLM) токена не видит ни в одном сценарии: ни в промпте, ни в аргументах инструментов, ни в MCP-протоколе. Для контраста: у MCP-сервера самого движка Cognopolis токен — аргумент каждого tool-а, потому что тот сервер многопользовательский; наша обёртка однопользовательская, и токен целиком уехал в окружение.
